...
export const applicationConfig = async (page: any) => {
  ...
  const rowContentLayer = await findElementInPaginatedTable(page, 'Test Config Layer Playwright')
  ...
  while (!elementFoundApp) {
    try {
      ...
      const pageButton = page.getByText(pageNumberApp.toString(), { exact: true }).first();
  ...
  await page.getByRole('button', { name: 'collapsed Karten', exact: true }).click();
