...
export const logs = async (page: any) => {
    ...
    await page.getByRole('row', { name: 'com.graphql-java INFO' }).locator('span').nth(2).click();
    ...
    await page.locator('div:nth-child(5) > .ant-select-item-option-content').first().click();
    await expect(page.getByText('Successfully set log level').first()).toBeVisible();
};
