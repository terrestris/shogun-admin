...
export const userMenu = async (page: any) => {
  ...
  const height = await image.evaluate(img => img.naturalHeight);
  ...
  const display = await page.locator('.application-info-modal').evaluate(el => getComputedStyle(el).display);
